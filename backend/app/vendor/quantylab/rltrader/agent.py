...
class Agent:
    ...
    def decide_action(self, pred_value, pred_policy, epsilon):
        ...
        # 유효한 행동만 후보로 둔다(액션 마스킹).
        #  - 보유 주식이 없으면 매도(SELL) 불가
        #  - 현금이 부족하면 매수(BUY) 불가
        #  - 관망(HOLD)은 항상 가능
        # 불가능한 매도를 계속 선택해 사실상 아무 거래도 못 하는 정책 붕괴를 방지한다.
        valid_actions = [a for a in self.ACTIONS if self.validate_action(a)]
        if not valid_actions:
            valid_actions = [Agent.ACTION_HOLD]

        if pred is None:
            # 예측 값이 없을 경우 탐험
            epsilon = 1
        else:
            # 유효 행동들의 예측값이 모두 같으면 탐험
            valid_preds = [pred[a] for a in valid_actions]
            if len(valid_preds) <= 1 or float(np.max(valid_preds) - np.min(valid_preds)) < 1e-8:
                epsilon = 1
